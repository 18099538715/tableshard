 ...
 * @类描述：db按照区间分库，即除以传入参数dbShardParam得到的整数，table固定个数分表， 即对传入tableShardParam取余
 * @创建人：Administrator
 * @创建时间：2017年6月21日 下午5:34:33
 * @company:步步高教育电子有限公司
 */
public class DbRegionTableNumFix extends AbstractTableDbShard {
	...
	public void setParam(Object object) {
		...
		if (getTablePrefix() == null || getTablePrefix() == "") {// 如果不分表
			DatabaseContextHolder.setDbName(getDbPrefix() + ((Long) args) / getDbShardParam());
			return;
		}
		DatabaseContextHolder.setShardParam(getTablePrefix() + ((Long) args) % getTableShardParam(), getDbPrefix() + ((Long) args) / getDbShardParam());
	}
